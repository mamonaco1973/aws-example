...
resource "aws_iam_role" "challenge_ec2_role" {
  name = "challenge_ec2_role"                   # IAM Role name
...
resource "aws_iam_policy" "challenge_access_policy" {
  name        = "challenge_access_policy"       # IAM Policy name
...
resource "aws_iam_role_policy_attachment" "attach_access_policy" {
  role       = aws_iam_role.challenge_ec2_role.name  # IAM Role to attach the policy
  policy_arn = aws_iam_policy.challenge_access_policy.arn  # Policy ARN to attach
}
