...
resource "aws_autoscaling_policy" "scale_up_policy" {
  ...
  autoscaling_group_name = aws_autoscaling_group.challenge_flask_asg.name  # Associated Auto Scaling Group
}
...
resource "aws_autoscaling_group" "challenge_flask_asg" {
  ...
  launch_template {
    id      = aws_launch_template.challenge_launch_template.id    # Launch template ID
    version = "$Latest"                                           # Latest version of the launch template
  }

  name                   = "challenge-flask-asg"                  # Auto Scaling Group name
  vpc_zone_identifier    = [                                      # Subnets for the ASG
    aws_subnet.challenge-subnet-1.id,
    aws_subnet.challenge-subnet-2.id
  ]
  ...
  target_group_arns = [aws_lb_target_group.challenge_alb_tg.arn]  # Associated ALB target group

  # Tags for instances created by the ASG
  tag {
    key                 = "Name"                                  # Tag key
    value               = "challenge-asg-instance"                # Tag value
