...
resource "aws_lb" "ecs_lb" {
  ...
  security_groups    = [data.aws_security_group.challenge_sg_http.id]
  subnets            = [data.aws_subnet.challenge_subnet_1.id, data.aws_subnet.challenge_subnet_2.id]
}
...
resource "aws_lb_target_group" "ecs_tg" {
  ...
  port        = 8000
  protocol    = "HTTP"
  vpc_id      = data.aws_vpc.challenge_vpc.id
  target_type = "ip"
...
resource "aws_launch_template" "ecs_lt" {
  ...
  network_interfaces {
    ...
    security_groups = [
      data.aws_security_group.challenge_sg_flask.id,
      data.aws_security_group.challenge_sg_https.id
    ]
  }
}
...
resource "aws_autoscaling_group" "ecs_asg" {
  ...
  max_size            = 2
  desired_capacity    = 2
  vpc_zone_identifier = [data.aws_subnet.challenge_subnet_1.id, data.aws_subnet.challenge_subnet_2.id]
...
resource "aws_ecs_service" "ecs_service" {
  ...
  network_configuration {
    subnets         = [data.aws_subnet.challenge_subnet_1.id, data.aws_subnet.challenge_subnet_2.id]
    security_groups = [data.aws_security_group.challenge_sg_flask.id]
  }
